...
const GUI = (() => {
  ...
  function displayBook(bookObj, rowIndex) {
    ...
    const percentCompleted = parseInt(
      (bookObj.currentpage / bookObj.totalpages) * 100
    );
    ...
    tempNode
      .querySelector(".row-currentpg")
      .addEventListener("input", updateProgressBar);
    tempNode
      .querySelector(".row-totalpg")
      .addEventListener("input", updateProgressBar);

    // add event listeners to buttons
    tempNode
      .querySelector(".deletebtn")
      .addEventListener("click", RemoveFromTable);
  ...
  function RemoveFromTable(e) {
    ...
    let rowIndex = parseInt(
      currentRow.querySelector(".row-counter").textContent
    );
  ...
  function refreshLibraryArray(rowElement) {
    ...
    const rowIndex = parseInt(
      rowElement.querySelector(".row-counter").textContent
    );
    ...
    myLibrary.editBook(
      rowIndex,
      new Book(newTitle, newAuthor, newCurrentPage, newTotalPage)
    );
  }
  ...
  function isValidRow(rowElement) {
    const titleInput = rowElement.querySelector(".row-title.input-field");
    const authorInput = rowElement.querySelector(".row-author.input-field");

    //reset validity messages for all input fields
    rowElement.querySelectorAll('input').forEach(input=>input.setCustomValidity(""));

    if (!titleInput.checkValidity()) {
      titleInput.setCustomValidity("Title must be at least 3 character long");
      titleInput.reportValidity();
      return false;
    }

    if (!authorInput.checkValidity()) {
      authorInput.setCustomValidity("Author must be at least 3 character long");
    ...
    if (newCurrentPage > newTotalPage) {
      currentPgInput.setCustomValidity(
        "Current page must be less than or equal to total pages."
      );
      totalPgInput.setCustomValidity(
        "Current page must be less than or equal to total pages."
      );
