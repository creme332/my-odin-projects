...
test("full app rendering/navigating", () => {
  ...
  expect(
    screen.getByText(/Healthy Snacking made delicious/i)
  ).toBeInTheDocument();
...
test("home page", () => {
  ...
  expect(screen.getByRole("heading").textContent).toMatch(
    /Healthy Snacking made delicious/i
  );
});

test.skip("search bar", () => {
  render(<Products />, { wrapper: BrowserRouter });

  // type 'natu'
  act(() => {
    userEvent.type(
      screen.getByRole("combobox", { name: /search bar/i }),
      "natu"
    );
  ...
  fireEvent.change(screen.getByRole("combobox", { name: /search bar/i }), {
    target: { value: "" },
  });

  // type 'bar'
  act(() => {
    userEvent.type(screen.getByRole("combobox"), "bar");
  });
  expect([...document.querySelectorAll(".card")].length).toEqual(1);
  ...
  // type 'lim'
  act(() => {
    userEvent.type(screen.getByRole("combobox"), "lim");
  });
  expect([...document.querySelectorAll(".card")].length).toEqual(1);
...
test("add to cart", async () => {
  ...
  expect(
    screen.getByRole("button", { name: "Checkout - Rs 15" }).textContent
  ).toMatch(/Checkout - Rs 15/i);
});
